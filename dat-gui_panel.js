...
(function () {
	...
	_this.init = function (setupDefaultItems, params) {
		...
		// Add width parameter to reduce control panel width
		params = params || {};
		params.width = 100; // Default is 245px, adjust this value as needed

		panel = new dat.GUI(params);
		panel.domElement.parentElement.style.zIndex = 2000;   // display the panel on the front of labels
		
		// Position panel on the top-left and set width for both panel and container
		panel.domElement.parentElement.style.position = 'absolute';
		panel.domElement.parentElement.style.left = '2px';
		panel.domElement.parentElement.style.top = '2px';
		panel.domElement.parentElement.style.right = 'auto';
		
		// Force width on both the container and the panel
		panel.domElement.parentElement.style.width = params.width + 'px';
		panel.domElement.style.width = params.width + 'px';
		...
		style.textContent = `
			@media screen and (max-width: 600px) {
				...
				.dg.main .close-button {
					width: ${params.width - 1}px !important;
				}
			}
		`;
